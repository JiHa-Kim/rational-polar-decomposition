...
Fresh current-`HEAD` benchmark on this machine with shared $\ell_0 = 10^{-3}$ and the default `spectral_bound` normalizer:
...
`dwh2` is 1.70x faster by median runtime and lower on `q_fro_error` in 11/11 default cases.
