...
class CaseGenerator:
    ...
    @classmethod
    def make_case(cls, name: str, m: int, n: int, device: torch.device, seed: int) -> torch.Tensor:
        if name == "gaussian":
        ...
        if name == "ar1_cols":
            ...
            powers = torch.pow(torch.tensor(rho, device=device, dtype=x.dtype), 
...
class MetricsSuite:
    ...
    @classmethod
    def ortho_stats(cls, q: torch.Tensor, workspace: dwh2.DWH2Workspace) -> tuple[float, float]:
        transposed = q.shape[0] < q.shape[1]
        Q = q.mT if transposed else q
        m, n = Q.shape
        gram, xbuf, scratch, tmp = workspace.buf, workspace.xbuf, workspace.scratch, workspace.tmp
        gram.zero_()
    ...
    @classmethod
    def polar_p_stats(cls, a_norm: torch.Tensor, q: torch.Tensor, gram_norm: torch.Tensor, workspace: dwh2.DWH2Workspace) -> tuple[float, float]:
        transposed = a_norm.shape[0] < a_norm.shape[1]
        X, Q = (a_norm.mT, q.mT) if transposed else (a_norm, q)
        m, _n = X.shape
        P, xbuf, scratch, tmp = workspace.buf, workspace.xbuf, workspace.scratch, workspace.tmp
        P.zero_()
        br = int(workspace.block_rows)
...
class BenchmarkRunner:
    ...
    def __init__(self, device: torch.device, args):
        self.device = device
        self.args = args
        self.ws_cache: OrderedDict[tuple[int, str, int | None], dwh2.DWH2Workspace] = OrderedDict()
    ...
    def set_fast_matmul(self) -> None:
        ...
        torch.set_float32_matmul_precision("high" if not self.args.no_tf32 else "highest")

    def measure(self, fn: Callable[[], object], trials: int, warmup: int) -> tuple[object, list[float]]:
    ...
    def get_workspace(self, n: int, dtype_str: str) -> dwh2.DWH2Workspace:
        ...
        ws = dwh2.DWH2Workspace.allocate(n, self.device, dtype, block_rows=dwh2.GRAM_BLOCK_ROWS)
...
class RegressionSuite:
    ...
    def check(baseline_path, current_path, time_thresh=0.10, metric_thresh=0.05):
        def load(p):
            ...
            with open(p, "r", encoding="utf-8") as f:
                for ln in f:
                    if ln.strip():
                        r = json.loads(ln)
                        k = (r["method"], r["case"], r["shape"], r["dtype"], r["tf32"], r["compile"])
                        res[k].append(r)
            return res
        ...
        for k in all_keys:
            if k not in base or k not in curr: continue
            for m in ["median_ms", "ortho_fro", "p2_gram_rel_fro"]:
                ...
                diff = (c_val - b_val) / b_val if b_val != 0 else (0 if c_val == 0 else float('inf'))
                thresh = time_thresh if m == "median_ms" else metric_thresh
                if diff > thresh:
                    regressions.append((k, m, diff))
                    print(f"{k[0]:<6} {k[1]:<20} {k[2]:<12} {m:<15} {b_val:<10.4f} {c_val:<10.4f} {diff*100:>+7.1f}% [REGRESSION]")
                elif diff < -thresh:
                    improvements.append((k, m, diff))
                    print(f"{k[0]:<6} {k[1]:<20} {k[2]:<12} {m:<15} {b_val:<10.4f} {c_val:<10.4f} {diff*100:>+7.1f}% [IMPROVED]")
        
        if regressions: print(f"\nDetected {len(regressions)} regressions.")
        elif not improvements: print("\nNo significant regressions found.")
        if improvements: print(f"Detected {len(improvements)} improvements.")


def import_gns(gns_path: str):
    root = os.path.dirname(os.path.abspath(__file__))
    paths = [os.path.join(root, "third_party", "gram-newton-schulz"), os.path.join(root, "third_party", "quack")]
    if gns_path: paths.insert(0, os.path.abspath(gns_path))
    for p in paths:
...
def make_gns_runner(gns_mod, kernel: bool, reset: list[int]):
    try:
        try: mod = importlib.import_module("gram_newton_schulz.coefficients")
        except: mod = importlib.import_module("newton_schulz.coefficients")
        coefs = getattr(mod, "POLAR_EXPRESS_COEFFICIENTS")
    except: coefs = getattr(gns_mod, "POLAR_EXPRESS_COEFFICIENTS", None)
    gns = getattr(gns_mod, "GramNewtonSchulz")(ns_use_kernels=kernel, ns_coefficients=coefs, gram_newton_schulz_reset_iterations=reset)
    def core(x: torch.Tensor) -> torch.Tensor:
        if x.dtype != torch.float16: x = x.half()
        t = x.size(-2) > x.size(-1)
        if t: x = x.mT.contiguous()
        xb = x.unsqueeze(0)
...
def main():
    ...
    ap.add_argument("--compile-mode", default="max-autotune", choices=["reduce-overhead", "max-autotune"])
    ap.add_argument("--trials", type=int, default=15)
    ap.add_argument("--warmup", type=int, default=5)
    ap.add_argument("--seeds", default="0,1")
    ap.add_argument("--dtype", default="fp16", choices=["fp16", "bf16", "fp32"])
    ap.add_argument("--cases", default="gaussian,lognormal_cols,ar1_cols,duplicate_cols,lowrank_noise,ill_conditioned,heavy_tail_t,sparse_like,orthogonal_noisy,rank_1_heavy,adversarial_condition")
    ...
    ap.add_argument("--hard", action="store_true", help="Run only hard cases (ill_conditioned, lowrank_noise)")
    ...
    ap.add_argument("--load-gns", type=str, default="stable/gns_baseline.jsonl", help="Load GNS results from file if skipped")
    args = ap.parse_args()
    if args.hard:
    ...
    warnings.filterwarnings("ignore", category=FutureWarning, message=r".*torch\._prims_common\.check.*")
    
    device = torch.device(args.device)
    if device.type == "cuda" and device.index is not None: torch.cuda.set_device(device.index)
    ...
    shapes = [(int(s.lower().split("x")[0]), int(s.lower().split("x")[1]), s) for s in args.shapes.split(",") if s.strip()]
    
    gns_core = None
    if not args.no_gns:
        gns_mod = import_gns(args.gns_path)
        gns_core = make_gns_runner(gns_mod, args.gns_use_kernels, [int(x) for x in args.gns_reset_iters.split(",") if x.strip()])
    ...
    def run_one(m, n, case, seed, method, fn_base, ws):
        a = CaseGenerator.make_case(case, m, n, device, seed).to({"fp16": torch.float16, "bf16": torch.bfloat16, "fp32": torch.float32}[args.dtype])
        a_norm, g_norm = dwh2.normalize_moment_with_small_gram(a, workspace=ws, inplace=True)
        if method == "dwh2": fn = lambda: fn_base(a_norm, g_norm, params=params, workspace=ws)
        else: fn = lambda: dwh2.PolarResult(q=fn_base(a_norm))
    ...
    try:
                ...
                for case in cases:
                    for seed in seeds:
                        ...
                        for name, core in methods:
                            out, a_norm, g_norm, med, mn = run_one(m, n, case, seed, name, core, ws)
                            res = {"method": name, "case": case, "shape": s_str, "dtype": args.dtype, "tf32": not args.no_tf32, "compile": args.compile, "trials": args.trials, "warmup": args.warmup, "median_ms": med, "min_ms": mn}
                            if not args.no_metrics:
                                ...
                                res.update({"ortho_fro": ortho, "ortho_max_abs": o_max, "p_skew_rel_fro": skew, "p2_gram_rel_fro": p2g, "chol_calls": st.calls, "chol_shifted_calls": st.shifted_calls, "chol_total_retries": st.total_retries, "chol_max_jitter": st.max_jitter})
        ...
        out_f.close()
        if args.baseline: RegressionSuite.check(args.baseline, args.output)
