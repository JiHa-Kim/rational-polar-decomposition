...
def main():
    ...
    if args.method == "dwh2":
        ...
        def fn(x):
            return dwh2(x, ell0=PAPER_MUON_ELL)
    else:

        def fn(x):
            return pe5(x, ell0=PAPER_MUON_ELL, coeffs=coeffs)
    ...
    for s in sorted(stats.values(), key=lambda x: x.total_time_us, reverse=True):
        ...
        print(
            f"{s.name:<30} | {s.total_time_ms / args.iters:>10.2f} | {count_per_run:>7.1f} | {s.mean_time_ms:>12.4f} | {share:>10.2f}%"
        )
